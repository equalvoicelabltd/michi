...
const SHOPPERS: Shopper[] = [
  {
    id: 1, name: 'Yuki Tanaka', icon: '🌸', location: '東京', experience: '8年',
...
export default function MichiMarketplace() {
  // ✅ Get current locale for correct URLs (e.g. /en/products, /zh/products)
  ...
  const categoryBadge: Record<string, string> = {
    fashion:     'bg-pink-50 text-pink-700 border border-pink-200',
    beauty:      'bg-rose-50 text-rose-700 border border-rose-200',
    electronics: 'bg-blue-50 text-blue-700 border border-blue-200',
    home:        'bg-amber-50 text-amber-700 border border-amber-200',
    anime:       'bg-purple-50 text-purple-700 border border-purple-200',
    food:        'bg-green-50 text-green-700 border border-green-200',
    general:     'bg-stone-700 text-stone-300 border border-stone-600',
  };
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6]">
      ...
      {/* ══════════════════════════════════════════════════════
          SECTION 1 — 代購買手市場
      ══════════════════════════════════════════════════════ */}
      <section id="market" className="max-w-7xl mx-auto px-6 lg:px-8 py-24 space-y-16">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
          <div className="space-y-3">
            ...
            <p className="text-stone-500 text-sm max-w-lg leading-relaxed">
              認識日本各地的專業代購員。每位職人均通過 Michi 認證，確保每次購物安心可靠。
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="relative">
              ...
              />
              <span className="absolute right-3 top-3 text-stone-400 text-sm pointer-events-none">🔍</span>
            </div>
        ...
        {displayedShoppers.length === 0 ? (
          <p className="text-stone-400 text-center py-16 text-sm">找不到符合條件的代購職人</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-16">
            {displayedShoppers.map((shopper) => (
              <div key={shopper.id} className="group">
                ...
                {/* Info card */}
                <div className="space-y-4 bg-white p-6 border border-stone-200 border-t-0">
                  <div className="flex justify-between items-end border-b border-stone-100 pb-2">
                    ...
                    <span className="font-serif italic text-[#C5A059]">{shopper.score}</span>
                  </div>
                  ...
                  {/* ✅ FIX: <a> with mailto, NOT bare <button> */}
                  <a
        ...
        {/* ✅ FIX: "更多代購職人" uses locale-aware <a> link */}
      ...
      <section id="products" className="bg-[#1C1C1C] text-white py-24">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 space-y-16">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            ...
            {/* AI refresh button — this one triggers scrape, NOT a link, so <button> is correct */}
            <button
            ...
            >
              {scraping ? <><span className="animate-spin inline-block">⟳</span> AI 搜尋中…</> : <>⟳ AI 更新商品</>}
            </button>
          </div>
          ...
          {loadingProducts ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="bg-stone-800 animate-pulse h-72" />
              ))}
            </div>
          ) : productError ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {products.map((product) => (
                <div key={product.id} className="bg-stone-900 border border-stone-800 hover:border-stone-600 transition-all group flex flex-col">

                  {/* Thumbnail */}
                  <div className="aspect-video bg-stone-800 overflow-hidden relative flex-shrink-0">
                    {product.image_url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <span className="text-6xl opacity-10 font-serif">道</span>
                      </div>
                    )}
                    {product.category && (
                      <span className={`absolute top-3 left-3 text-[9px] font-black uppercase tracking-widest px-2 py-1 ${categoryBadge[product.category.toLowerCase()] ?? categoryBadge.general}`}>
                        {product.category}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="p-6 flex flex-col gap-3 flex-grow">
                    ...
                    {product.ai_summary && (
                      <div className="border-l-2 border-[#C5A059] pl-3">
                        ...
                        <p className="text-xs text-stone-300 leading-relaxed line-clamp-3">{product.ai_summary}</p>
                      </div>
                    )}

                    {!product.ai_summary && product.description && (
                      <p className="text-xs text-stone-400 leading-relaxed line-clamp-3">{product.description}</p>
                    )}

                    {/* Footer row */}
                    <div className="mt-auto pt-4 border-t border-stone-800 flex justify-between items-center gap-2">
                      <span className="text-[9px] font-bold text-stone-500 uppercase tracking-widest truncate">
                        {product.source ?? 'Japan'} · {formatDate(product.published_at)}
                      </span>

                      {/* ✅ FIX: "了解更多" → <a> with real href, NOT <button> */}
                      {product.source_url ? (
                        <a
                          href={product.source_url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex-shrink-0 text-[9px] font-black uppercase tracking-widest text-[#C5A059] hover:text-white transition-colors"
                        >
                          了解更多 ↗
                        </a>
                      ) : (
