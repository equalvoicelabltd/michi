...
const ALL_LOCALES = ['zh-CN', 'zh', 'en', 'ja'];
        ...
        <div className="absolute right-0 mt-2 w-52 bg-white rounded-xl shadow-xl border border-stone-200 z-20 overflow-hidden">
          {languages.map((lang) => (
            <button key={lang.code} onClick={() => handleChange(lang.code)}
              className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-stone-50 transition text-left ${locale === lang.code ? 'bg-stone-50' : ''}`}>
              <span className="text-lg">{lang.flag}</span>
              <span className="text-sm font-medium flex-1 text-stone-700">{lang.name}</span>
              {locale === lang.code && (<svg className="w-4 h-4 text-[#1A237E]" fill="currentColor" viewBox="0 0 20 20">
